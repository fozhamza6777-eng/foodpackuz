...
### 2.9-qadam: Profil rasmi, tug'ilgan sana, jins va ommaviy oferta (o'ninchi migratsiya)

1. **SQL Editor**'da yana **New query** tugmasini bosing.
2. `supabase/migrations/0010_avatar_and_offer.sql` faylining **butun mazmunini** nusxalab
   joylashtiring.
3. **Run** tugmasini bosing.

Bu skript quyidagilarni qo'shadi:
- `profiles` jadvaliga `avatar_url`, `birth_date`, `gender`, `accepted_offer_at` ustunlari
- Profil rasmlari uchun alohida, ochiq `avatars` bucket'i — har bir mijoz FAQAT o'z rasmini
  yuklashi/o'zgartirishi mumkin (boshqa birovning rasmini almashtira olmaydi)

**Muhim:** ro'yxatdan o'tish endi "Ommaviy oferta shartlariga roziman" belgisini talab qiladi.
Bu matn `lib/publicOfferText.ts` faylida — **haqiqiy yuridik matningiz bilan almashtirishni**
...
Mijoz profili endi zamonaviy mobil-ilova uslubida qurilgan:

- **Yuqori gradient qism** — profil rasmi (yoki ismning bosh harflari), ism-familiya, telefon
  raqami va sozlamalar (⚙️) tugmasi.
- **Statistika kartochkasi** — nechta buyurtma berilgani va jami qancha xarid qilingani bir
  qarashda ko'rinadi, bosilsa "Buyurtmalarim" ro'yxati ochiladi.
- **Menyu ro'yxati**: Buyurtmalarim, Bekor qilingan buyurtmalar (alohida ro'yxat), Yoqtirganlarim
  (sevimlilar paneli ochiladi), Sharhlarim (mijoz barcha mahsulotlarga yozgan sharhlari bitta
  joyda), Filiallarim.
- **Sozlamalar** (gear tugmasi orqali alohida ochiladi) — profil rasmini yuklash/o'zgartirish/
  o'chirish, ism va tashkilot nomini tahrirlash, **ixtiyoriy** tug'ilgan sana va jins (ro'yxatdan
  o'tishda so'ralmaydi, faqat mijoz o'zi xohlasa shu yerdan to'ldiradi), parolni almashtirish,
  hisobdan chiqish.

Profil rasmi Supabase'ning alohida, xavfsiz saqlash joyida (`avatars` bucket) turadi — har bir
mijoz FAQAT o'z rasmini yuklay oladi, boshqa birovning rasmini almashtira olmaydi.

## Ommaviy oferta

Ro'yxatdan o'tishda mijoz **"Ommaviy oferta shartlari bilan tanishdim va roziman"** belgisini
bosishi shart — bu belgisiz "Ro'yxatdan o'tish" tugmasi ishlamaydi. Oferta matni `/oferta`
sahifasida to'liq ko'rinadi va yangi oynada ochiladi. Rozilik bildirilgan payt (`accepted_offer_at`)
ma'lumotlar bazasida avtomatik saqlanadi.

**Juda muhim:** `lib/publicOfferText.ts` faylidagi matn — faqat **namuna (shablon)**. Uni haqiqiy
kompaniya rekvizitlaringiz va yurist tomonidan tekshirilgan shartlar bilan albatta almashtiring —
bu yuridik jihatdan muhim hujjat.
