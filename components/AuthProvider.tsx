...
interface AuthContextValue {
  ...
  updateExtraInfo: (input: { birthDate?: string; gender?: "male" | "female" }) => Promise<AuthResult>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateExtraInfo = useCallback(
    async (input: { birthDate?: string; gender?: "male" | "female" }): Promise<AuthResult> => {
      if (!session?.user) return { error: "Avval tizimga kiring." };
