...
function canRequestCancel(status: string) {
  return !["bekor", "bekor_sorovi", "yetkazildi"].includes(status);
}
...
export default function ProfileDrawer({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (isOpen && tab === "orders" && allProducts === null) {
      fetchActiveProducts().then(setAllProducts);
    }
  }, [isOpen, tab, auth.session, orders, allProducts]);
  ...
  const handleDirectCancel = async (order: OrderRow) => {
    if (!window.confirm("Buyurtmani bekor qilmoqchimisiz?")) return;
    setCancelSubmitting(true);
    setCancelError(null);
    const { error } = await supabase.from("orders").update({ status: "bekor" }).eq("id", order.id);
    setCancelSubmitting(false);
    if (error) {
      window.alert("Xatolik yuz berdi, qaytadan urinib ko'ring.");
      return;
    }
    setOrders((prev) => (prev ? prev.map((o) => (o.id === order.id ? { ...o, status: "bekor" } : o)) : prev));
  };
                                ...
                                <div className="p-4 flex flex-col gap-2 bg-surface/60">
                                  ...
                                  {order.status === "bekor" && order.cancel_reason && (
                                    <p className="text-xs text-ink/40 font-medium">
                                      Bekor qilish sababi: {order.cancel_reason}
                                    </p>
                                  )}
                                      ...
                                      {cancelReasonOrderId === order.id ? (
                                        <div className="flex flex-col gap-2">
                                          <p className="text-xs text-ink/50 font-medium">
                                            Buyurtma berilganiga 1 soatdan ko'p vaqt o'tgan. Bekor qilish sababini
                                            yozing — buni administrator ko'rib chiqadi.
                                          </p>
                                          <textarea
                                          ...
                                          />
                                          {cancelError && (
                                            <p className="text-xs text-danger font-semibold">{cancelError}</p>
                                          )}
                                          <div className="flex gap-2">
                                            <button
                                              onClick={() => {
                                        ...
                                        <button
                                          onClick={() =>
                                            isWithinCancelWindow(order.created_at)
                                              ? handleDirectCancel(order)
                                              : setCancelReasonOrderId(order.id)
                                          }
                                          disabled={cancelSubmitting}
